...
function normalizeRecord(record = {}, fallbackApp = null, fallbackSlot = null) {
  ...
  out.complete = Boolean(out.integration_ready || (out.user && out.password));
  return out;
}

function integrationReady(item) {
  if (item.app_id === 'shinephone') return Boolean(item.api_token || (item.user && item.password));
...
export function integrationMissing(item) {
  ...
  if(item.app_id==='shinephone'){
    if(!item.api_token){ need(item.user,'USER'); need(item.password,'PASSWORD'); }
  }
