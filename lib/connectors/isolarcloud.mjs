...
/* ============================================================================
   HISTÓRICO DA USINA — MÊS / ANO
============================================================================ */

/*
 * O getPowerStationList da sua conta não entrega month_energy/year_energy.
 *
 * Por isso buscamos o histórico oficial do pseudo-dispositivo da usina:
 *
 * endpoint:
 * /openapi/getDevicePointsDayMonthYearDataList
 *
 * point:
 * p83022 = rendimento diário (Wh)
 *
 * query_type:
 * 2 = mensal
 * 3 = anual
 ...
 *
 * Para mês atual:
 * start_time/end_time = YYYYMM
 *
 * Para ano atual:
 * start_time/end_time = YYYY
 */


function currentPlantPeriods() {
...
function extractHistoricalTotalWh(
  result,
  psKey,
  pointId = '83022'
) {
  ...
  /*
   * Para query_type mensal/anual + data_type 4,
   * cada registro traz a chave "4" com o total agregado.
   *
   * Normalmente vem apenas um registro, mas somamos todos
   * para suportar respostas com mais de um intervalo.
   */
  let totalWh =
    0;
...
async function getPlantHistoricalEnergy(
  client,
  psId
) {
  ...
  async function query(
    ...
    try {
      ...
      const result =
        await client.post(
...

          {
            ...
            /*
             * 83022 = Rendimento diário da usina em Wh.
             * Em consulta mensal/anual pedimos o TOTAL agregado.
             */
            data_point:
              'p83022',
      ...
      /*
       * Histórico não pode derrubar a leitura da usina.
       * Mantém null e o restante continua normalmente.
       */
      return null;
    }
  }
...
export async function getISolarCloudData(
  ...
  return {
    ...
    note:
      'Dados da planta obtidos pela Monitoring API. Mês e ano são buscados pelo histórico oficial getDevicePointsDayMonthYearDataList usando p83022 e agregação total. Token gerado automaticamente pelo backend.'
  };
}
