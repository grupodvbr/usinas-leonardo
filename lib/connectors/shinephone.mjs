 ...
 *
 * OPCIONAL PARA VALORES EM R$ QUANDO A GROWATT NÃO DEVOLVER
 * RECEITA/TARIFA NA OPENAPI:
 *
 * SOLAR_SHINEPHONE_01_TARIFF_BRL
 *
 * Exemplo:
 * SOLAR_SHINEPHONE_01_TARIFF_BRL=0.95
 *
 * ALERTA:
 *
...
/* ============================================================================
   ...
   A OpenAPI V1 documenta /plant/details.
   Usamos esta chamada também para tentar obter:
   - moeda
   - tarifa
   - campos financeiros quando disponíveis na conta/região
============================================================================ */
...
async function getPlantDetails(
  ...
  try {
    ...
    /*
     * A ausência desta rota/campo financeiro NÃO pode derrubar
     * o monitoramento de geração.
     */
    return null;
  }
}
...
function parseMoneyNumber(
  ...
  /*
   * Remove símbolo/moeda e preserva apenas número, ponto,
   * vírgula e sinal. Trata formatos:
   *
   * 1234.56
   * 1,234.56
   * 1.234,56
  ...
  if (
    ...
    /*
     * O último separador é considerado decimal.
     */
    if (
      lastComma >
      lastDot
...
function getCurrency(
  ...
  /*
   * Alguns retornos Growatt embutem a unidade no texto:
   * "1234.56 (R$)".
   */
  const textCandidates = [
    plant?.plantMoneyText,
    plant?.eTotalMoneyText,
    plant?.etotalMoneyText,
    overview?.plantMoneyText,
    overview?.eTotalMoneyText,
    overview?.etotalMoneyText,
    details?.plantMoneyText,
    details?.eTotalMoneyText,
    details?.etotalMoneyText
  ];


  for (
    const value of textCandidates
  ) {
...
function envTariffBrl(
  ...
  const names = [
    ...
    `SOLAR_SHINEPHONE_${slot}_TARIFF_BRL`,

    /*
     * Alias global opcional.
     */
    'SOLAR_SHINEPHONE_TARIFF_BRL'
  ];
...
function getTariff(
  ...
  /*
   * Campos observados em respostas do ecossistema Growatt.
   */
  const tariff =
    pickMoney(
...
function moneyData({
  ...
  /*
   * Primeiro prioriza valores financeiros PRONTOS retornados
   * pela Growatt.
   */
  let today =
    pickMoney(
  ...
  /*
   * Se a Growatt não devolver um período monetário pronto,
   * podemos calculá-lo usando a tarifa da própria Growatt.
   * Como último fallback, pode usar a tarifa BRL configurada
   * na Vercel.
   */
  let calculated =
    false;
  ...
  /*
   * Tarifa manual é explicitamente em BRL.
   */
  if (
    !currency &&
    tariff.source ===
  ...
  if (anyMoney) {
    ...
    if (
      ...
      source =
        'growatt_or_calculated_from_growatt_tariff';
  ...
  return {
    ...
    /*
     * Campos prontos em BRL para o front.
     * Só são preenchidos quando temos certeza de que a moeda é BRL.
     */
    revenue_today_brl:
      currency === 'BRL'
        ? today
...
async function getByApiToken(
  ...
  return {
    ...
    note:
      'Growatt OpenAPI com geração, tentativa de receita/moeda/tarifa, fallback diário e alerta automático para estações offline.'
  };
}
...
async function getByLegacySession(
  ...
  for (
    ...
    const legacyTodayKwh =
      sum(
        'today_kwh'
      );


    const legacyMonthKwh =
      sum(
        'month_kwh'
      );


    const legacyTotalKwh =
  ...
  return {
    ...
    note:
      'Fallback por usuário/senha com leitura de geração e campos financeiros Growatt quando disponíveis.'
  };
}
